...
set bin_file "H:/testproject/project_2/preemptive.bin"
...
zero_words [expr {$ddr_load_addr + 0x4000}] 0x10000
...
puts "PREEMPTIVE_PSUART_BRIDGE_RELEASE_CPU"
...
for {set n 0} {$n < 12000} {incr n} {
    ...
    if {($v & 0x100) != 0} {
        ...
        incr printed
        set idle 0
        if {$printed >= 1600} { break }
    } else {
        after 1
        incr idle
        if {$printed > 0 && $idle > 8000} { break }
    }
}

puts ""
puts [format "PREEMPTIVE_PSUART_BRIDGE_PRINTED=%d" $printed]
...
read_dbg DBG_CLINT_MTIME 0xa4
read_dbg DBG_CLINT_MTIMECMP 0xa8
read_dbg DBG_CLINT_STATUS 0xac
disconnect
