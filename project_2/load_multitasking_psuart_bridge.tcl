...
set bin_file "H:/testproject/project_2/multitasking.bin"
...
proc read_dbg {name offset} {
    global dbg_base
    puts [format "%s=0x%08x" $name [read32 [expr {$dbg_base + $offset}]]]
}
...
proc zero_words {base_addr byte_count} {
    ...
    for {set addr $base_addr} {$addr < $end_addr} {incr addr 4} { mwr $addr 0x00000000 }
...
zero_words [expr {$ddr_load_addr + 0x3000}] 0x4000

puts [format "DDR_WORDS=0x%08x 0x%08x 0x%08x 0x%08x" [read32 $ddr_load_addr] [read32 [expr {$ddr_load_addr + 4}]] [read32 [expr {$ddr_load_addr + 8}]] [read32 [expr {$ddr_load_addr + 12}]]]
puts "MULTITASKING_PSUART_BRIDGE_RELEASE_CPU"
...
for {set n 0} {$n < 200000} {incr n} {
    ...
    if {($v & 0x100) != 0} {
        ...
        flush stdout
        append line_buf [format "%c" $ch]
        if {$ch == 10} {
            if {[string first "Running..." $line_buf] >= 0} { incr running_lines }
            set line_buf ""
        }
        incr printed
        set idle 0
        if {$running_lines >= 30} { break }
        if {$printed >= 5000} { break }
    } else {
        after 1
        incr idle
        if {$printed > 0 && $idle > 15000} { break }
    }
}

puts ""
puts [format "MULTITASKING_PSUART_BRIDGE_PRINTED=%d" $printed]
