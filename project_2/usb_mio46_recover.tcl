set out [open "H:/testproject/project_2/usb_mio46_recover.out" "w"]
...
puts $out "USB_MIO46_RECOVER_BEGIN"
...
show $out BEFORE_USBMODE $usb_mode
show $out BEFORE_GPIO_DATA1 $gpio_data1
show $out BEFORE_MIO46 $mio46

# Stop the PL RISC-V while PS-side USB registers are recovered.
...
mwr $otgsc [expr {([rd $otgsc] & ~0x007f0001) | 0x00000026}]
...
show $out AFTER_USBCMD $usb_cmd
show $out AFTER_USBSTS $usb_sts
show $out AFTER_ULPI $ulpi_view
show $out AFTER_PORTSC1 $portsc1
show $out AFTER_OTGSC $otgsc
show $out AFTER_USBMODE $usb_mode
show $out AFTER_GPIO_DATA1 $gpio_data1
show $out AFTER_MIO46 $mio46
