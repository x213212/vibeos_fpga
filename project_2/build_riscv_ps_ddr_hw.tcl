...
set use_incremental_checkpoint 0
set build_jobs 2
set enable_eth 0
set fclk_mhz 60
...
if {$enable_eth} {
    create_bd_cell -type ip -vlnv xilinx.com:ip:xlslice:1.0 eth_tx_nibble
    create_bd_cell -type ip -vlnv xilinx.com:ip:xlconcat:2.1 eth_rx_byte
    create_bd_cell -type ip -vlnv xilinx.com:ip:xlconstant:1.1 eth_rx_pad_zero
}
create_bd_cell -type module -reference riscv_ps_ddr_top riscv_0
if {!$enable_eth} {
    create_bd_cell -type ip -vlnv xilinx.com:ip:xlconstant:1.1 eth_dbg_zero1
    create_bd_cell -type ip -vlnv xilinx.com:ip:xlconstant:1.1 eth_dbg_zero4
    set_property -dict [list CONFIG.CONST_WIDTH {1} CONFIG.CONST_VAL {0}] [get_bd_cells eth_dbg_zero1]
    set_property -dict [list CONFIG.CONST_WIDTH {4} CONFIG.CONST_VAL {0}] [get_bd_cells eth_dbg_zero4]
}
...
if {$enable_eth} {
    set_property -dict [list CONFIG.DIN_WIDTH {8} CONFIG.DIN_FROM {3} CONFIG.DIN_TO {0} CONFIG.DOUT_WIDTH {4}] [get_bd_cells eth_tx_nibble]
    set_property -dict [list CONFIG.NUM_PORTS {2} CONFIG.IN0_WIDTH {4} CONFIG.IN1_WIDTH {4}] [get_bd_cells eth_rx_byte]
    set_property -dict [list CONFIG.CONST_WIDTH {4} CONFIG.CONST_VAL {0}] [get_bd_cells eth_rx_pad_zero]
}
...
if {$enable_eth} {
    ...
    set_property -dict [list CONFIG.FREQ_HZ 25000000] [get_bd_ports ENET0_GMII_RX_CLK_0]
    set_property -dict [list CONFIG.FREQ_HZ 25000000] [get_bd_ports ENET0_GMII_TX_CLK_0]
    ...
    create_bd_port -dir O -type rst ETH_RESET
    ...
    connect_bd_net [get_bd_pins processing_system7_0/ENET0_GMII_TXD] [get_bd_pins eth_tx_nibble/Din]
    connect_bd_net [get_bd_pins eth_tx_nibble/Dout] [get_bd_ports enet0_gmii_txd]
    connect_bd_net [get_bd_ports enet0_gmii_rxd] [get_bd_pins eth_rx_byte/In0]
    connect_bd_net [get_bd_pins eth_rx_pad_zero/dout] [get_bd_pins eth_rx_byte/In1]
    connect_bd_net [get_bd_pins eth_rx_byte/dout] [get_bd_pins processing_system7_0/ENET0_GMII_RXD]
    connect_bd_net [get_bd_pins processing_system7_0/FCLK_RESET0_N] [get_bd_ports ETH_RESET]
    connect_bd_net [get_bd_ports ENET0_GMII_TX_CLK_0] [get_bd_pins riscv_0/eth_dbg_tx_clk]
    connect_bd_net [get_bd_ports ENET0_GMII_RX_CLK_0] [get_bd_pins riscv_0/eth_dbg_rx_clk]
    connect_bd_net [get_bd_ports ENET0_GMII_RX_DV_0] [get_bd_pins riscv_0/eth_dbg_rx_dv]
    connect_bd_net [get_bd_ports enet0_gmii_rxd] [get_bd_pins riscv_0/eth_dbg_rxd]
    connect_bd_net [get_bd_ports ENET0_GMII_TX_EN_0] [get_bd_pins riscv_0/eth_dbg_tx_en]
    connect_bd_net [get_bd_ports enet0_gmii_txd] [get_bd_pins riscv_0/eth_dbg_txd]

    make_bd_intf_pins_external [get_bd_intf_pins processing_system7_0/MDIO_ETHERNET_0]
    set mdio_eth_bd_port [lindex [get_bd_intf_ports *MDIO_ETHERNET*] 0]
    if {$mdio_eth_bd_port eq ""} {
        error "Could not create external Ethernet MDIO BD interface"
    }
} else {
    connect_bd_net [get_bd_pins eth_dbg_zero1/dout] [get_bd_pins riscv_0/eth_dbg_tx_clk]
    connect_bd_net [get_bd_pins eth_dbg_zero1/dout] [get_bd_pins riscv_0/eth_dbg_rx_clk]
    connect_bd_net [get_bd_pins eth_dbg_zero1/dout] [get_bd_pins riscv_0/eth_dbg_rx_dv]
    connect_bd_net [get_bd_pins eth_dbg_zero1/dout] [get_bd_pins riscv_0/eth_dbg_tx_en]
    connect_bd_net [get_bd_pins eth_dbg_zero4/dout] [get_bd_pins riscv_0/eth_dbg_rxd]
    connect_bd_net [get_bd_pins eth_dbg_zero4/dout] [get_bd_pins riscv_0/eth_dbg_txd]
}
...
set bd_file [get_files $out_dir/riscv_ps_ddr_hw.srcs/sources_1/bd/riscv_ps_ddr/riscv_ps_ddr.bd]
# Vivado 2019.2 can hang here on Windows when BD/IP OOC child runs invoke
# rundef.js with "Access denied".  Build the block design inside top synthesis
# so the Ethernet-enabled bitmap does not depend on those child runs.
...
if {$enable_eth} {
    ...
    # Z7-LITE tutorial Ethernet pinmap. This is the verified mapping for the
    # current board: with the RTL8201F forced to MII mode it gives live
    # TX/RX clocks and answers ping at 192.168.0.154.
...
# Avoid project run wrappers (`launch_runs`/`wait_on_run`) on Windows.  Vivado
# 2019.2 can finish the child synthesis process but leave the parent stuck
# waiting forever.  Run the implementation flow in this Vivado process instead.
