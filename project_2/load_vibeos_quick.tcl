...
proc z7lite_usb_phy_reset_release {} {
    # Z7-LITE schematic: USB3320 RESETB is OTG_nRST on PS_MIO46, active low.
    ...
    mwr $gpio_data1 [expr {[mrd -value $gpio_data1] | $bit}]
    after 500
    puts [format "USB_PHY_RESET_RELEASE MIO46=0x%08x GPIO_DATA1=0x%08x DIRM1=0x%08x OEN1=0x%08x" \
...
proc usb0_host_release {} {
    ...
    if {[lindex $wake 0]} {
        ...
        set otgctrl [usb_ulpi_wait_idle $ulpi_view 200]
        mwr $ulpi_view 0x60040041
        set funcctrl [usb_ulpi_wait_idle $ulpi_view 200]
        mwr $ulpi_view 0x60070000
        set ifctrl [usb_ulpi_wait_idle $ulpi_view 200]
        mwr $ulpi_view 0x600b0060
        set drv [usb_ulpi_wait_idle $ulpi_view 200]
        set ulpi_ok [expr {[lindex $otgctrl 0] && [lindex $funcctrl 0] && [lindex $ifctrl 0] && [lindex $drv 0]}]
    }
    mwr $portsc1 [expr {[mrd -value $portsc1] | 0x00001000}]
    mwr $usb_sts [mrd -value $usb_sts]
    mwr $usb_cmd 0x00080001
    after 500
    puts [format "USB0_HOST_RELEASE CMD=0x%08x STS=0x%08x PORT=0x%08x MODE=0x%08x OTG0=0x%08x OTG=0x%08x ULPI0=0x%08x ULPI=0x%08x ULPI_OK=%d" \
        [mrd -value $usb_cmd] [mrd -value $usb_sts] [mrd -value $portsc1] [mrd -value $usb_mode] \
        $otg0 [mrd -value $otgsc] $ulpi0 [mrd -value $ulpi_view] $ulpi_ok]
}
...
puts [format "DOW_BINARY_FILE=%s DDR_LOAD_ADDR=0x%08x FIRST_WORD=0x%08x" $bin_file $ddr_load_addr [expr {$first_word & 0xffffffff}]]
after 100
mwr $cpu_reset_gpio 0x00000001
after 3000
puts "QUICK_LOAD_DONE"
disconnect
