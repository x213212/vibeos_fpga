...
psa_status_t psa_crypto_init(void)
{
    ...
    mbedtls_printf("[BOOT] psa_crypto_init: driver_wrappers\n");
    status = mbedtls_psa_crypto_init_subsystem(PSA_CRYPTO_SUBSYSTEM_DRIVER_WRAPPERS);
    mbedtls_printf("[BOOT] psa_crypto_init: driver_wrappers status=%d\n", (int) status);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    mbedtls_printf("[BOOT] psa_crypto_init: key_slots\n");
    status = mbedtls_psa_crypto_init_subsystem(PSA_CRYPTO_SUBSYSTEM_KEY_SLOTS);
    mbedtls_printf("[BOOT] psa_crypto_init: key_slots status=%d\n", (int) status);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    mbedtls_printf("[BOOT] psa_crypto_init: rng\n");
    status = mbedtls_psa_crypto_init_subsystem(PSA_CRYPTO_SUBSYSTEM_RNG);
    mbedtls_printf("[BOOT] psa_crypto_init: rng status=%d\n", (int) status);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    mbedtls_printf("[BOOT] psa_crypto_init: transaction\n");
    status = mbedtls_psa_crypto_init_subsystem(PSA_CRYPTO_SUBSYSTEM_TRANSACTION);
    mbedtls_printf("[BOOT] psa_crypto_init: transaction status=%d\n", (int) status);
