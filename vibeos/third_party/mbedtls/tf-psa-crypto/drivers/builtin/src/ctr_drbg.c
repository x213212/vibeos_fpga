...
{
    mbedtls_printf("[BOOT] ctr_drbg_init ctx=%p\n", (void *) ctx);
...
{
    mbedtls_printf("[BOOT] ctr_drbg_free ctx=%p\n", (void *) ctx);
    if (ctx == NULL) {
        return;
    }
                          ...
                          size_t len)
{
    ...
    mbedtls_printf("[BOOT] ctr_drbg_seed ctx=%p entropy=%p p_entropy=%p custom=%p len=%u\n",
                   (void *) ctx, (void *) f_entropy, p_entropy, (void *) custom,
                   (unsigned) len);
    ...
    mbedtls_printf("[BOOT] ctr_drbg_seed: psa_setup begin\n");
    status = ctr_drbg_setup_psa_context(&ctx->psa_ctx, key, MBEDTLS_CTR_DRBG_KEYSIZE);
    if (status != PSA_SUCCESS) {
        ret = psa_generic_status_to_mbedtls(status);
        mbedtls_printf("[BOOT] ctr_drbg_seed: psa_setup failed status=%d ret=%d\n",
                       (int) status, ret);
        return status;
    }
    mbedtls_printf("[BOOT] ctr_drbg_seed: psa_setup ok\n");
#else
    if ((ret = mbedtls_aes_setkey_enc(&ctx->aes_ctx, key,
                                      MBEDTLS_CTR_DRBG_KEYBITS)) != 0) {
        mbedtls_printf("[BOOT] ctr_drbg_seed: aes_setkey_enc ret=%d\n", ret);
        return ret;
    }
#endif

    /* Do the initial seeding. */
    mbedtls_printf("[BOOT] ctr_drbg_seed: reseed_internal begin nonce_len=%u\n",
                   (unsigned) nonce_len);
    if ((ret = mbedtls_ctr_drbg_reseed_internal(ctx, custom, len,
                                                nonce_len)) != 0) {
        mbedtls_printf("[BOOT] ctr_drbg_seed: reseed_internal ret=%d\n", ret);
        return ret;
    }
    mbedtls_printf("[BOOT] ctr_drbg_seed: success\n");
    return 0;
}
