                                               ...
                                               const mbedtls_mpi *N, mbedtls_mpi *prec_RR)
{
    ...
    lib_printf("[LIBSSH2][MPI] exp_mod enter Nbits=%u Ebits=%u Nn=%u En=%u public=%d\n",
               (unsigned) mbedtls_mpi_bitlen(N),
               (unsigned) mbedtls_mpi_bitlen(E),
               (unsigned) N->n,
               (unsigned) E->n,
               E_public);

    if (mbedtls_mpi_cmp_int(N, 0) <= 0 || (N->p[0] & 1) == 0) {
        lib_printf("[LIBSSH2][MPI] exp_mod bad N sign/odd check\n");
        return MBEDTLS_ERR_MPI_BAD_INPUT_DATA;
    }

    if (mbedtls_mpi_cmp_int(E, 0) < 0) {
        lib_printf("[LIBSSH2][MPI] exp_mod bad E sign check\n");
        return MBEDTLS_ERR_MPI_BAD_INPUT_DATA;
    }

    if (mbedtls_mpi_bitlen(E) > MBEDTLS_MPI_MAX_BITS ||
        mbedtls_mpi_bitlen(N) > MBEDTLS_MPI_MAX_BITS) {
        lib_printf("[LIBSSH2][MPI] exp_mod bitlen exceeds max\n");
        return MBEDTLS_ERR_MPI_BAD_INPUT_DATA;
    }
    ...
    if (E->n == 0) {
        ...
        lib_printf("[LIBSSH2][MPI] exp_mod zero exponent ret=%d\n", ret);
        return ret;
    }
    ...
    lib_printf("[LIBSSH2][MPI] exp_mod work limbs=%u\n", (unsigned) T_limbs);
    mbedtls_mpi_uint *T = (mbedtls_mpi_uint *) mbedtls_calloc(T_limbs, sizeof(mbedtls_mpi_uint));
    if (T == NULL) {
        lib_printf("[LIBSSH2][MPI] exp_mod calloc failed\n");
        return MBEDTLS_ERR_MPI_ALLOC_FAILED;
    }
    ...
    if (prec_RR == NULL || prec_RR->p == NULL) {
        ...
        if (ret != 0) {
            lib_printf("[LIBSSH2][MPI] exp_mod precompute RR failed ret=%d\n", ret);
            goto cleanup;
        }
        lib_printf("[LIBSSH2][MPI] exp_mod precompute RR ok\n");

        if (prec_RR != NULL) {
            *prec_RR = RR;
        }
    } else {
        ret = mbedtls_mpi_grow(prec_RR, N->n);
        if (ret != 0) {
            lib_printf("[LIBSSH2][MPI] exp_mod reuse RR grow failed ret=%d\n", ret);
            goto cleanup;
        }
        RR = *prec_RR;
        lib_printf("[LIBSSH2][MPI] exp_mod reused RR\n");
    }
    ...
    if (ret != 0) {
        lib_printf("[LIBSSH2][MPI] exp_mod copy A failed ret=%d\n", ret);
        goto cleanup;
    }
    lib_printf("[LIBSSH2][MPI] exp_mod copy A ok\n");
    ...
    if (mbedtls_mpi_cmp_mpi(X, N) >= 0) {
        ret = mbedtls_mpi_mod_mpi(X, X, N);
        if (ret != 0) {
            lib_printf("[LIBSSH2][MPI] exp_mod reduced A mod N failed ret=%d\n", ret);
            goto cleanup;
        }
        lib_printf("[LIBSSH2][MPI] exp_mod reduced A mod N\n");
    }
    ret = mbedtls_mpi_grow(X, N->n);
    if (ret != 0) {
        lib_printf("[LIBSSH2][MPI] exp_mod grow X failed ret=%d\n", ret);
        goto cleanup;
    }
    lib_printf("[LIBSSH2][MPI] exp_mod grow X ok\n");
     ...
     */
    {
        ...
        if (E_public == MBEDTLS_MPI_IS_PUBLIC) {
            ...
            lib_printf("[LIBSSH2][MPI] exp_mod core unsafe done\n");
        } else {
            mbedtls_mpi_core_exp_mod(X->p, X->p, N->p, N->n, E->p, E->n, RR.p, T);
            lib_printf("[LIBSSH2][MPI] exp_mod core safe done\n");
        }
        mbedtls_mpi_core_from_mont_rep(X->p, X->p, N->p, N->n, mm, T);
        lib_printf("[LIBSSH2][MPI] exp_mod from mont done\n");
    }
...
cleanup:
    ...
    lib_printf("[LIBSSH2][MPI] exp_mod exit ret=%d\n", ret);
    return ret;
}
