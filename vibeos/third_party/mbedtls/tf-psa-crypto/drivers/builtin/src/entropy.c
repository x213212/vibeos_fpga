...
{
    mbedtls_printf("[BOOT] entropy_init ctx=%p\n", (void *) ctx);
    ctx->source_count = 0;
...
{
    mbedtls_printf("[BOOT] entropy_free ctx=%p\n", (void *) ctx);
    if (ctx == NULL) {
        return;
    }
...
static int entropy_gather_internal(mbedtls_entropy_context *ctx)
{
    ...
    mbedtls_printf("[BOOT] entropy_gather_internal ctx=%p sources=%d\n",
                   (void *) ctx, ctx ? ctx->source_count : -1);
...
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_printf("[BOOT] entropy_gather ctx=%p\n", (void *) ctx);
...
int mbedtls_entropy_func(void *data, unsigned char *output, size_t len)
{
    ...
    mbedtls_printf("[BOOT] entropy_func ctx=%p output=%p len=%u\n",
                   (void *) ctx, (void *) output, (unsigned) len);
...
exit:
    ...
    mbedtls_printf("[BOOT] entropy_func ret=%d\n", ret);
    return ret;
}
