...
{
    printf("[JITDBG] putdefs p=%lx\n", (unsigned long)(uintptr_t)p);
    if ((uintptr_t)p < 4096) {
        printf("[JITDBG] putdefs invalid pointer p=%lx\n", (unsigned long)(uintptr_t)p);
        return;
    }
    while (*p)
...
static void tcc_predefs(TCCState *s1, CString *cs, int is_asm)
{
    ...
    printf("[JITDBG] predefs target_machine_defs=%lx target_os_defs=%lx\n",
           (unsigned long)(uintptr_t)target_machine_defs,
           (unsigned long)(uintptr_t)target_os_defs);
